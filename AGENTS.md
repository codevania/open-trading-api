...
- 사용자가 "오늘 리포트", "데일리 리포트", "일일 리포트", "장마감 리포트", "루틴 돌려줘"처럼 요청하면 `_report/routines/daily-report-routine.md`를 따라 일일 투자 리포트를 생성한다.
- 일일 리포트는 `_report/daily/YYYY/YYYY-MM-DD.md`에 작성한다.
