...
Use the 72-date KRX OpenAPI market-data merge, merged KIND status replay, local status coverage audit, `Point-in-Time Universe` smoke, 20-day `Point-in-Time` Liquidity Filter smoke, paper-only Momentum Signal Candidate smoke, Signal forward-return smoke, long-only portfolio target smoke, Backtest input contract validation, Backtest PnL smoke with KOSPI benchmark-excess diagnostics, Backtest cost/benchmark assumptions validation, benchmark return smoke, and local Quant readiness check as the plumbing baseline. The latest coverage audit is still `hold`: two KIND current snapshots now produce `497` logical status-event rows across `2` raw capture dates and replay cleanly across `198137` market-data rows, but `managed_issue`, `market_alert`, and `trading_halt` still have active-like rows with `0` release/resume-like rows, and no source coverage manifest covers the required status types. The audit pass gate now also requires every lifecycle status type with active-like rows to have release/resume-like rows, dated raw capture evidence, and a source coverage manifest covering the market-data window before `historical_complete` can pass. Backtest cost/benchmark assumptions are now `pass_assumption_only`, and benchmark returns are only `pass_smoke`; neither is Backtest readiness. The next lane is to obtain historical transition coverage by date/source, produce the source coverage manifest, resolve remaining `UNKNOWN` market rows only where official evidence supports it, extend the market-data window enough to cover production forward-return horizons, wire actual fees plus benchmark attribution into a real Backtest engine, and keep the Universe/Liquidity/Signal/portfolio/contract/PnL/assumption/benchmark/readiness smoke aligned until it can become a Backtest input. KIS demo trading is only at local preflight level; do not build or run an order executor until demo auth/account, buying-power, sellable-quantity, status/cancel, kill-switch, and explicit confirmation gates are implemented.
...
- Latest 72-date baseline artifacts:
...
- Signal forward-return smoke is implemented; it joined the `2120` Signal Candidate rows to local future close prices across `1,5` trading-day horizons and produced `4240` diagnostic rows: `4000` complete and `240` `missing_forward_price`.
...
- Backtest PnL smoke is implemented; the latest 1-day horizon diagnostic report is `pass_smoke` with `1060` rows, `1040` complete rows, `1040` KOSPI benchmark joined rows, `20` missing forward prices due to the short smoke window, and average weighted excess vs benchmark of `0.0171%`.
...
- Benchmark return smoke is implemented; the latest local report produced `318` KOSPI/KOSDAQ/KOSPI200 horizon rows, with `300` complete and `18` missing forward index rows due to the short smoke window.
...
15. Extend the KRX OpenAPI market-data window further before attempting production Momentum lookbacks.
...
- Forward-return smoke has partial coverage only because the current market-data window ends on `2025-04-18`; production horizons need additional future trading dates beyond the latest Signal Candidate date.
...
- Benchmark return smoke is only a comparison input diagnostic; its KOSPI join into PnL smoke is still not production benchmark attribution.
