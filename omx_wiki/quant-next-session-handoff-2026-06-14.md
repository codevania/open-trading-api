...
- Quant readiness check is implemented; the latest 20-day local report with merged status coverage, lifecycle gap evidence, and attribution smoke marks market-data window `pass`, Liquidity Filter `pass_smoke`, Signal Candidate `pass_smoke`, forward-return smoke `pass_smoke`, portfolio target smoke `pass_smoke`, Backtest input contract `pass_smoke`, Backtest PnL smoke `pass_smoke`, Backtest assumptions `pass_assumption_only`, benchmark returns smoke `pass_smoke`, Backtest attribution smoke `pass_smoke_assumption_only`, Point-in-Time status coverage `hold` with source manifest validation `not_supplied` and lifecycle missing release/resume groups `304` (`managed_issue=104`, `market_alert=70`, `trading_halt=130`), Backtest engine `hold`, live trading controls `blocked`, and KIS demo account `blocked`.
...
15. Re-run [[scripts/quant_readiness_check.py|scripts/quant_readiness_check.py]] after each Point-in-Time, Liquidity Filter, Signal Candidate, forward-return, portfolio-target, Backtest contract, PnL smoke, assumption, benchmark, attribution, or KIS account milestone.
16. Extend KRX OpenAPI market-data further only when the signal/status window expands or production Momentum lookbacks require it; do not treat price-only extension as status coverage.
17. Keep result as paper/smoke only until full `Point-in-Time` status replay is solved.
