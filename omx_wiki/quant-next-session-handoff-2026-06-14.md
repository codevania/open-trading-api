...
- Point-in-Time status coverage audit is implemented; the latest merged audit is `hold` because coverage mode is `current_snapshot_smoke`, raw status capture dates are only `2` (`2026-07-03..2026-07-08`), release/resume-like event rows are `0`, `managed_issue` / `market_alert` / `trading_halt` have no release/resume rows, and no source coverage manifest covers `managed_issue`, `trading_halt`, `market_alert`, and `delisting` even though replay matched `198137/198137` market rows.
- Point-in-Time source coverage manifest validation is implemented; run [[scripts/quant_point_in_time_status_source_manifest_validate.py|scripts/quant_point_in_time_status_source_manifest_validate.py]] before passing any filled manifest into the coverage audit.
...
2. Obtain historical status transition coverage across the selected rebalance range, then fill and validate the source coverage manifest; extra current snapshots help evidence preservation but do not solve release/resume lifetimes by themselves.
