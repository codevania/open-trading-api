...
- Point-in-Time Universe smoke is implemented; 62-date replayed market-data rows produced `159048` include and `11519` exclude rows.
- Point-in-Time Liquidity Filter smoke is implemented; 62-date replayed market-data rows with a 20-day rule produced `44235` include and `126332` exclude rows, with `109890` rows evaluated on the full 20-day lookback.
- Point-in-Time Momentum Signal Candidate smoke is implemented; 62-date, 20-day Momentum over the Liquidity rows produced `1720` paper-only candidates across `43` candidate dates: `860` BUY candidates and `860` SELL candidates. This is not a Backtest result and does not generate order intents.
- Signal forward-return smoke is implemented; it joined the `1720` Signal Candidate rows to local future close prices across `1,5` trading-day horizons and produced `3440` diagnostic rows: `3200` complete and `240` `missing_forward_price`.
...
- Backtest PnL smoke is implemented; the latest 1-day horizon diagnostic report is `pass_smoke` with `860` rows, `840` complete rows, `840` KOSPI benchmark joined rows, `20` missing forward prices due to the short smoke window, and average weighted excess vs benchmark of `-0.0043%`.
...
- Benchmark return smoke is implemented; the latest local report produced `258` KOSPI/KOSDAQ/KOSPI200 horizon rows, with `240` complete and `18` missing forward index rows due to the short smoke window.
...
- Forward-return smoke has partial coverage only because the current market-data window ends on `2025-04-04`; production horizons need additional future trading dates beyond the latest Signal Candidate date.
