...
- Quant readiness check is implemented; the latest 20-day local report with merged status coverage, lifecycle gap evidence, attribution smoke, OOS/WF preflight, and Bias Control preflight marks market-data window `pass`, Liquidity Filter `pass_smoke`, Signal Candidate `pass_smoke`, forward-return smoke `pass_smoke`, portfolio target smoke `pass_smoke`, Backtest input contract `pass_smoke`, Backtest PnL smoke `pass_smoke`, Backtest assumptions `pass_assumption_only`, benchmark returns smoke `pass_smoke`, Backtest attribution smoke `pass_smoke_assumption_only`, OOS/Walk-Forward preflight `pass_smoke_plumbing_only`, Bias Control preflight `hold`, Point-in-Time status coverage `hold` with source manifest validation `not_supplied` and lifecycle missing release/resume groups `304` (`managed_issue=104`, `market_alert=70`, `trading_halt=130`), Backtest engine `hold`, live trading controls `blocked`, and KIS demo account `blocked`.
...
17. Re-run [[scripts/quant_readiness_check.py|scripts/quant_readiness_check.py]] after each Point-in-Time, Liquidity Filter, Signal Candidate, forward-return, portfolio-target, Backtest contract, PnL smoke, assumption, benchmark, attribution, OOS/WF preflight, Bias Control preflight, or KIS account milestone.
...
- Latest readiness check [[_report/quant/research/2026-07-09-quant-readiness-check-20d-20250102-20250418-price-through-20250502-bias-preflight|_report/quant/research/2026-07-09-quant-readiness-check-20d-20250102-20250418-price-through-20250502-bias-preflight.md]] confirms the same state: Backtest `hold`, live trading `blocked`.
