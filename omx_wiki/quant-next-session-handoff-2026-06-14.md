...
Use the 72-date KRX OpenAPI market-data merge, merged KIND status replay, local status coverage audit, `Point-in-Time Universe` smoke, 20-day `Point-in-Time` Liquidity Filter smoke, paper-only Momentum Signal Candidate smoke, price-through-`2025-05-02` Signal forward-return smoke, long-only portfolio target smoke, Backtest input contract validation, price-through Backtest PnL smoke with KOSPI benchmark-excess diagnostics, Backtest cost/benchmark assumptions validation, price-through benchmark return smoke, Backtest attribution smoke, OOS/Walk-Forward preflight, Bias Control preflight, and local Quant readiness check as the plumbing baseline. The latest coverage audit is still `hold`: two KIND current snapshots now produce `497` logical status-event rows across `2` raw capture dates, 81-date local market enrichment reduces merged `UNKNOWN` market labels from `48` to `47`, and the replay matches `198137/198137` market-data rows, but `managed_issue`, `market_alert`, and `trading_halt` still have active-like rows with `0` release/resume-like rows, and no filled source coverage manifest covers the required status types. The lifecycle gap report narrows this to `304` code/status collection targets with `missing_release_resume_evidence` (`managed_issue=104`, `market_alert=70`, `trading_halt=130`), the source manifest draft narrows the manifest-filling work to `10` `pending_raw_evidence` rows across `managed_issue`, `trading_halt`, `market_alert`, and `delisting`, and the evidence collection plan consolidates the full Point-in-Time status queue into `361` pending rows: `source_manifest_evidence=10`, `release_resume_evidence=304`, and `market_label_resolution=47`. The audit pass gate now also requires every lifecycle status type with active-like rows to have release/resume-like rows, dated raw capture evidence, and a source coverage manifest that passes source-policy/raw-path validation and covers the market-data window before `historical_complete` can pass. Backtest attribution now has a local `pass_smoke_assumption_only` report with baseline/stress cost and KOSPI active-return diagnostics, OOS/Walk-Forward preflight now has `pass_smoke_plumbing_only` fold evidence, and Bias Control preflight exists, but `broker_fee_override_required=true` and Bias Control status `hold` mean this is still not production Backtest/OOS/Bias Control evidence. The next lane is to execute the evidence collection plan against official raw KRX/KIND sources, fill and validate the source coverage manifest from official raw evidence, resolve remaining `47` `UNKNOWN` market rows only where official evidence supports it, replace assumed costs with actual KIS fees, wire production benchmark attribution into a real Backtest engine, and keep the Universe/Liquidity/Signal/portfolio/contract/PnL/assumption/benchmark/attribution/OOS/Bias/readiness smoke aligned until it can become a Backtest input. KIS demo trading is only at local preflight level; do not build or run an order executor until demo auth/account, buying-power, sellable-quantity, status/cancel, kill-switch, and explicit confirmation gates are implemented.
...
- Point-in-Time status coverage audit is implemented; the latest merged audit is `hold` because coverage mode is `current_snapshot_smoke`, raw status capture dates are only `2` (`2026-07-03..2026-07-08`), release/resume-like event rows are `0`, `managed_issue` / `market_alert` / `trading_halt` have no release/resume rows, and source coverage manifest validation is `not_supplied` for `managed_issue`, `trading_halt`, `market_alert`, and `delisting` even though replay matched `198137/198137` market rows.
...
- Point-in-Time lifecycle gap reporting is implemented; [[_report/quant/research/2026-07-09-point-in-time-status-lifecycle-gaps-20250102-20250418-merged-snapshots-market-enriched-81d|_report/quant/research/2026-07-09-point-in-time-status-lifecycle-gaps-20250102-20250418-merged-snapshots-market-enriched-81d.md]] records `304` code/status collection targets with `missing_release_resume_evidence` and keeps Backtest readiness at `hold`.
- Point-in-Time status evidence collection planning is implemented; [[_report/quant/research/2026-07-09-point-in-time-status-evidence-collection-plan-20250102-20250418-market-enriched-81d|_report/quant/research/2026-07-09-point-in-time-status-evidence-collection-plan-20250102-20250418-market-enriched-81d.md]] records `361` pending rows split into `source_manifest_evidence=10`, `release_resume_evidence=304`, and `market_label_resolution=47`. This is a collection plan only, not source coverage evidence.
- Point-in-Time status-event market enrichment is implemented; the second snapshot resolved `297/327` rows from the 42-date market-data join and left `30` as `UNKNOWN`. The merged set has `497` rows across `286` event codes; 81-date local market enrichment resolves `1` additional market label and leaves `47` `UNKNOWN` market rows across `31` codes. The unknown-market target report splits them into `trading_halt=28`, `delisting=9`, `managed_issue=6`, and `market_alert=4`.
...
- Backtest attribution smoke is implemented; [[_report/quant/research/2026-07-09-backtest-attribution-smoke-20d-20250102-20250418-price-through-20250502|_report/quant/research/2026-07-09-backtest-attribution-smoke-20d-20250102-20250418-price-through-20250502.md]] records `53` date-horizon rows as `pass_smoke_assumption_only`, average gross return `0.3758%`, average baseline net return `0.0458%`, average stress net return `-0.3242%`, and average baseline active return `0.0280%`; `broker_fee_override_required=true` keeps this assumption-only.
- OOS/Walk-Forward preflight is implemented; [[_report/quant/research/2026-07-09-oos-walk-forward-preflight-20d-20250102-20250418-price-through-20250502|_report/quant/research/2026-07-09-oos-walk-forward-preflight-20d-20250102-20250418-price-through-20250502.md]] records `3` temporal fold rows as `pass_smoke_plumbing_only`, average test baseline net return `-0.0827%`, average test baseline active return `0.0466%`, and OOS readiness `hold`.
- Bias Control preflight is implemented; [[_report/quant/research/2026-07-09-bias-control-preflight-20d-20250102-20250418-price-through-20250502|_report/quant/research/2026-07-09-bias-control-preflight-20d-20250102-20250418-price-through-20250502.md]] records `10` checks with final status `hold`, including `survivorship_bias=hold`, `cost_bias=hold`, `overfitting=hold`, and `execution_bias=blocked`.
- Quant readiness check is implemented; the latest 20-day local report with merged status coverage, lifecycle gap evidence, evidence collection plan, attribution smoke, OOS/WF preflight, and Bias Control preflight marks market-data window `pass`, Liquidity Filter `pass_smoke`, Signal Candidate `pass_smoke`, forward-return smoke `pass_smoke`, portfolio target smoke `pass_smoke`, Backtest input contract `pass_smoke`, Backtest PnL smoke `pass_smoke`, Backtest assumptions `pass_assumption_only`, benchmark returns smoke `pass_smoke`, Backtest attribution smoke `pass_smoke_assumption_only`, OOS/Walk-Forward preflight `pass_smoke_plumbing_only`, Bias Control preflight `hold`, Point-in-Time status evidence collection plan `plan_only` with `361` pending rows, Point-in-Time status coverage `hold` with source manifest validation `not_supplied` and lifecycle missing release/resume groups `304` (`managed_issue=104`, `market_alert=70`, `trading_halt=130`), Backtest engine `hold`, live trading controls `blocked`, and KIS demo account `blocked`.
...
2. Use the evidence collection plan rows from [[_report/quant/research/2026-07-09-point-in-time-status-evidence-collection-plan-20250102-20250418-market-enriched-81d.rows.csv|_report/quant/research/2026-07-09-point-in-time-status-evidence-collection-plan-20250102-20250418-market-enriched-81d.rows.csv]] as collection targets, obtain historical status transition coverage across the selected rebalance range, then fill the source coverage manifest with official `source_url`, `raw_path`, and `confidence` evidence and let the coverage audit validate it; extra current snapshots help evidence preservation but do not solve release/resume lifetimes by themselves.
3. Resolve remaining `47` `UNKNOWN` KIND market rows from [[_report/quant/research/2026-07-09-kind-status-events-unknown-market-targets-market-enriched-81d.rows.csv|_report/quant/research/2026-07-09-kind-status-events-unknown-market-targets-market-enriched-81d.rows.csv]] only where an official source or deterministic join beyond the current market-data enrichment can support it.
...
17. Re-run [[scripts/quant_readiness_check.py|scripts/quant_readiness_check.py]] after each Point-in-Time, evidence collection plan, Liquidity Filter, Signal Candidate, forward-return, portfolio-target, Backtest contract, PnL smoke, assumption, benchmark, attribution, OOS/WF preflight, Bias Control preflight, or KIS account milestone.
...
- Latest readiness check [[_report/quant/research/2026-07-09-quant-readiness-check-20d-20250102-20250418-price-through-20250502-evidence-plan|_report/quant/research/2026-07-09-quant-readiness-check-20d-20250102-20250418-price-through-20250502-evidence-plan.md]] confirms the same state: evidence collection plan `plan_only`, Point-in-Time status coverage `hold`, Backtest `hold`, live trading `blocked`.
