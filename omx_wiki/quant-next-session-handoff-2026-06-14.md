...
Use the 72-date KRX OpenAPI market-data merge, merged KIND status replay, local status coverage audit, `Point-in-Time Universe` smoke, 20-day `Point-in-Time` Liquidity Filter smoke, paper-only Momentum Signal Candidate smoke, price-through-`2025-05-02` Signal forward-return smoke, long-only portfolio target smoke, Backtest input contract validation, price-through Backtest PnL smoke with KOSPI benchmark-excess diagnostics, Backtest cost/benchmark assumptions validation, price-through benchmark return smoke, and local Quant readiness check as the plumbing baseline. The latest coverage audit is still `hold`: two KIND current snapshots now produce `497` logical status-event rows across `2` raw capture dates, 81-date local market enrichment reduces merged `UNKNOWN` market labels from `48` to `47`, and the replay matches `198137/198137` market-data rows, but `managed_issue`, `market_alert`, and `trading_halt` still have active-like rows with `0` release/resume-like rows, and no filled source coverage manifest covers the required status types. The lifecycle gap report narrows this to `304` code/status collection targets with `missing_release_resume_evidence` (`managed_issue=104`, `market_alert=70`, `trading_halt=130`), and the source manifest draft narrows the manifest-filling work to `10` `pending_raw_evidence` rows across `managed_issue`, `trading_halt`, `market_alert`, and `delisting`. The audit pass gate now also requires every lifecycle status type with active-like rows to have release/resume-like rows, dated raw capture evidence, and a source coverage manifest that passes source-policy/raw-path validation and covers the market-data window before `historical_complete` can pass. Backtest cost/benchmark assumptions are now `pass_assumption_only`, and benchmark/PnL outputs are still smoke-only even though forward prices are complete for the existing 72-date signal window. The next lane is to obtain historical transition coverage by date/source, fill and validate the source coverage manifest from official raw evidence, resolve remaining `47` `UNKNOWN` market rows only where official evidence supports it, wire actual fees plus benchmark attribution into a real Backtest engine, and keep the Universe/Liquidity/Signal/portfolio/contract/PnL/assumption/benchmark/readiness smoke aligned until it can become a Backtest input. KIS demo trading is only at local preflight level; do not build or run an order executor until demo auth/account, buying-power, sellable-quantity, status/cancel, kill-switch, and explicit confirmation gates are implemented.
...
14. Re-run [[scripts/quant_readiness_check.py|scripts/quant_readiness_check.py]] after each Point-in-Time, Liquidity Filter, Signal Candidate, forward-return, portfolio-target, Backtest contract, PnL smoke, assumption, benchmark, or KIS account milestone.
15. Extend KRX OpenAPI market-data further only when the signal/status window expands or production Momentum lookbacks require it; do not treat price-only extension as status coverage.
16. Keep result as paper/smoke only until full `Point-in-Time` status replay is solved.
