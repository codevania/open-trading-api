...
2. Use the lifecycle gap rows as collection targets, obtain historical status transition coverage across the selected rebalance range, then fill the source coverage manifest and let the coverage audit validate it; extra current snapshots help evidence preservation but do not solve release/resume lifetimes by themselves.
