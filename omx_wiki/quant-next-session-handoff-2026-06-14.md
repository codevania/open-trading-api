...
- Quant readiness check is implemented; the latest 20-day local report with merged 3-snapshot status coverage, lifecycle gap evidence, evidence collection plan, attribution smoke, OOS/WF preflight, and Bias Control preflight marks market-data window `pass`, Liquidity Filter `pass_smoke`, Signal Candidate `pass_smoke`, forward-return smoke `pass_smoke`, portfolio target smoke `pass_smoke`, Backtest input contract `pass_smoke`, Backtest PnL smoke `pass_smoke`, Backtest assumptions `pass_assumption_only`, benchmark returns smoke `pass_smoke`, Backtest attribution smoke `pass_smoke_assumption_only`, OOS/Walk-Forward preflight `pass_smoke_plumbing_only`, Bias Control preflight `hold`, Point-in-Time status evidence collection plan `plan_only` with `385` pending rows, Point-in-Time status coverage `hold` with source manifest validation `not_supplied` and lifecycle missing release/resume groups `314` (`managed_issue=106`, `market_alert=78`, `trading_halt=130`), Backtest engine `hold`, live trading controls `blocked`, and KIS demo account `blocked`.
...
2. Use the evidence collection plan rows from [[_report/quant/research/2026-07-09-point-in-time-status-evidence-collection-plan-20250102-20250418-merged-3snapshots-market-enriched-81d.rows.csv|_report/quant/research/2026-07-09-point-in-time-status-evidence-collection-plan-20250102-20250418-merged-3snapshots-market-enriched-81d.rows.csv]] as collection targets, obtain historical status transition coverage across the selected rebalance range, then fill the source coverage manifest with official `source_url`, `raw_path`, and `confidence` evidence and let the coverage audit validate it; extra current snapshots help evidence preservation but do not solve release/resume lifetimes by themselves.
