...
- Point-in-Time status coverage audit is implemented; the latest merged audit is `hold` because coverage mode is `current_snapshot_smoke`, raw status capture dates are only `2` (`2026-07-03..2026-07-08`), release/resume-like event rows are `0`, and `managed_issue` / `market_alert` / `trading_halt` have no release/resume rows even though replay matched `170567/170567` market rows.
