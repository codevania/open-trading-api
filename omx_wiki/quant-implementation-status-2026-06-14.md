...
- Overall Quant implementation progress: `78-81%`
- Current Snapshot Universe v0 progress: `85-90%`
- Backtest readiness: `hold`
- Live trading readiness: `blocked`
- Current phase: `extended_market_data_backtest_input_smoke`

The project is beyond planning and now has a usable current-snapshot Universe artifact, a saved-raw Liquidity Filter smoke artifact, a Universe-based OHLCV request queue, the first 360 read-only KIS captured Universe rows, approved KRX OpenAPI core market-data services with raw collection, normalization, continuity auditing, date-scoped market-data joining, and a 33-date market-data merge. Local `Point-in-Time` status-event schema/config/replay/coverage-audit scaffolding is implemented, KRX Data Marketplace status screens are mapped but unattended JSON probes returned `auth_required`, and a KIND public current snapshot has been normalized into `344` valid status-event rows, market-enriched for `310/344` rows after both 17-date and 33-date joins, replayed against the 33-date market-data merge, converted into a `Point-in-Time Universe` smoke with `84644` include / `6034` exclude rows, filtered through a 20-day `Point-in-Time` Liquidity Filter smoke with `14292` include / `76386` exclude rows, converted into `520` paper-only 20-day Momentum Signal Candidate rows, evaluated with local forward-return, portfolio-target, Backtest input contract, Backtest PnL smoke, and summarized by a local Quant readiness check. KIS demo trading has local preflight only and is blocked by missing `KIS_PAPER_STOCK` in the ignored MCP `.env.kis`. It is still not Backtest-ready because status coverage audit remains `hold` under `current_snapshot_smoke`, production Momentum lookbacks, and Backtest/OOS/Bias Control are incomplete.
...
- KIND current snapshot status events replayed against the 33-date KRX OpenAPI market-data merge and marked `552/90678` rows as `exclude_by_status_event`.
...
- [[_report/quant/research/2026-07-06-point-in-time-status-coverage-audit-20250102-20250221|_report/quant/research/2026-07-06-point-in-time-status-coverage-audit-20250102-20250221.md]] records coverage status `hold`: mode `current_snapshot_smoke`, replay matched `90678/90678` rows, rows with applied status event `552`, rows excluded by status event `552`, and release/resume-like event rows `0`.
...
- The latest 33-date `Point-in-Time Universe` smoke produced `84644` include rows and `6034` exclude rows.
...
- The latest 33-date, 20-day `Point-in-Time` Liquidity Filter smoke produced `14292` include rows and `76386` exclude rows, with `35792` rows evaluated on the full 20-day lookback.
...
- The latest 33-date, 20-day Momentum Signal Candidate smoke produced `520` rows across `13` candidate dates: `260` BUY candidates and `260` SELL candidates.
- The latest Signal forward-return smoke produced `1040` horizon rows across `1,5` trading-day horizons: `800` complete and `240` missing forward prices.
- The latest Signal portfolio target smoke produced `260` long-only paper target rows across `13` rebalance dates at `5%` target weight each.
...
- The latest Backtest PnL smoke is `pass_smoke` with `260` rows, `240` complete rows, and `20` missing forward prices on the latest Signal Candidate date.
...
- [[_report/quant/research/2026-07-06-signal-forward-return-smoke-20d-20250102-20250221|_report/quant/research/2026-07-06-signal-forward-return-smoke-20d-20250102-20250221.md]] records `1040` horizon rows across `1,5` trading-day horizons: `800` complete and `240` missing forward prices because the smoke window still ends too soon.
