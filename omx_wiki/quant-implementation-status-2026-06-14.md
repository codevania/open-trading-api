...
- The two-snapshot merged status-event set has `497` logical event rows and `48` `UNKNOWN` market rows; the latest 62-date replay marked `1227/170567` rows as `exclude_by_status_event`.
...
- Merged KIND status events replayed against the 62-date KRX OpenAPI market-data merge and marked `1227/170567` rows as `exclude_by_status_event`.
...
- [[_report/quant/research/2026-07-08-point-in-time-status-coverage-audit-20250102-20250404-merged-snapshots|_report/quant/research/2026-07-08-point-in-time-status-coverage-audit-20250102-20250404-merged-snapshots.md]] records coverage status `hold`: mode `current_snapshot_smoke`, replay matched `170567/170567` rows, rows with any status-event code `15770`, rows with applied status event `1227`, rows excluded by status event `1227`, raw status capture dates `2`, release/resume-like event rows `0`, and source coverage manifest rows `0`. Lifecycle diagnostics show `managed_issue` `105/0`, `market_alert` `75/0`, and `trading_halt` `253/0` active-like/release-resume rows.
...
- The latest 62-date `Point-in-Time Universe` smoke produced `159048` include rows and `11519` exclude rows.
...
- The latest 62-date, 20-day `Point-in-Time` Liquidity Filter smoke produced `44235` include rows and `126332` exclude rows, with `109890` rows evaluated on the full 20-day lookback.
...
- The latest 62-date, 20-day Liquidity + 5-day Momentum Signal Candidate smoke produced `1720` rows across `43` candidate dates: `860` BUY candidates and `860` SELL candidates.
- The latest Signal forward-return smoke produced `3440` horizon rows across `1,5` trading-day horizons: `3200` complete and `240` missing forward prices.
- The latest Signal portfolio target smoke produced `860` long-only paper target rows across `43` rebalance dates at `5%` target weight each.
...
- The latest Backtest PnL smoke is `pass_smoke` with `860` rows, `840` complete rows, `840` KOSPI benchmark joined rows, `20` missing forward prices on the latest Signal Candidate date, and average weighted excess vs benchmark of `-0.0043%`.
...
- [[_report/quant/research/2026-07-08-benchmark-return-smoke-20d-20250102-20250404|_report/quant/research/2026-07-08-benchmark-return-smoke-20d-20250102-20250404.md]] records `258` benchmark horizon rows for KOSPI, KOSDAQ, and KOSPI200: `240` complete and `18` `missing_forward_index`.
- [[_report/quant/research/2026-07-08-benchmark-return-smoke-20d-20250102-20250404.rows.csv|_report/quant/research/2026-07-08-benchmark-return-smoke-20d-20250102-20250404.rows.csv]] stores the machine-readable benchmark return smoke rows.
...
- [[_report/quant/research/2026-07-08-signal-forward-return-smoke-20d-20250102-20250404|_report/quant/research/2026-07-08-signal-forward-return-smoke-20d-20250102-20250404.md]] records `3440` horizon rows across `1,5` trading-day horizons: `3200` complete and `240` missing forward prices because the smoke window still ends too soon.
