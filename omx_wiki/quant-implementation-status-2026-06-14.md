...
- [[_report/quant/research/2026-07-08-point-in-time-status-coverage-audit-20250102-20250307|_report/quant/research/2026-07-08-point-in-time-status-coverage-audit-20250102-20250307.md]] records coverage status `hold`: mode `current_snapshot_smoke`, replay matched `115447/115447` rows, rows with applied status event `705`, rows excluded by status event `705`, raw status capture dates `1`, and release/resume-like event rows `0`. Lifecycle diagnostics show `managed_issue`, `market_alert`, and `trading_halt` all have active-like rows but `0` release/resume-like rows.
