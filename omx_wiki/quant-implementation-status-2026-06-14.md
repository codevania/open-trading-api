...
- The two-snapshot merged status-event set has `497` logical event rows and `48` `UNKNOWN` market rows; the latest 52-date replay marked `889/142990` rows as `exclude_by_status_event`.
...
- Merged KIND status events replayed against the 52-date KRX OpenAPI market-data merge and marked `889/142990` rows as `exclude_by_status_event`.
...
- [[_report/quant/research/2026-07-08-point-in-time-status-coverage-audit-20250102-20250321-merged-snapshots|_report/quant/research/2026-07-08-point-in-time-status-coverage-audit-20250102-20250321-merged-snapshots.md]] records coverage status `hold`: mode `current_snapshot_smoke`, replay matched `142990/142990` rows, rows with any status-event code `13220`, rows with applied status event `889`, rows excluded by status event `889`, raw status capture dates `2`, and release/resume-like event rows `0`. Lifecycle diagnostics show `managed_issue` `105/0`, `market_alert` `75/0`, and `trading_halt` `253/0` active-like/release-resume rows.
...
- The latest 52-date `Point-in-Time Universe` smoke produced `133465` include rows and `9525` exclude rows.
...
- The latest 52-date, 20-day `Point-in-Time` Liquidity Filter smoke produced `34408` include rows and `108582` exclude rows, with `84414` rows evaluated on the full 20-day lookback.
...
- The latest 52-date, 20-day Liquidity + 5-day Momentum Signal Candidate smoke produced `1320` rows across `33` candidate dates: `660` BUY candidates and `660` SELL candidates.
- The latest Signal forward-return smoke produced `2640` horizon rows across `1,5` trading-day horizons: `2400` complete and `240` missing forward prices.
- The latest Signal portfolio target smoke produced `660` long-only paper target rows across `33` rebalance dates at `5%` target weight each.
...
- The latest Backtest PnL smoke is `pass_smoke` with `660` rows, `640` complete rows, and `20` missing forward prices on the latest Signal Candidate date.
...
- [[_report/quant/research/2026-07-08-signal-forward-return-smoke-20d-20250102-20250321|_report/quant/research/2026-07-08-signal-forward-return-smoke-20d-20250102-20250321.md]] records `2640` horizon rows across `1,5` trading-day horizons: `2400` complete and `240` missing forward prices because the smoke window still ends too soon.
