...
- [[scripts/quant_readiness_check.py|scripts/quant_readiness_check.py]] summarizes local Quant gates without calling KIS/KRX or generating order intents and links the local status coverage audit report into the Point-in-Time status coverage gate, including source manifest validation status when present.
- [[_report/quant/research/2026-07-08-quant-readiness-check-20d-20250102-20250418-price-through-20250502|_report/quant/research/2026-07-08-quant-readiness-check-20d-20250102-20250418-price-through-20250502.md]] records the latest gate state: market-data window `pass`, Liquidity Filter `pass_smoke`, Signal Candidate `pass_smoke`, forward-return smoke `pass_smoke`, portfolio target smoke `pass_smoke`, Backtest input contract `pass_smoke`, Backtest PnL smoke `pass_smoke`, Backtest assumptions `pass_assumption_only`, benchmark returns smoke `pass_smoke`, Point-in-Time status coverage `hold` with source manifest validation `not_supplied`, Backtest engine `hold`, live trading controls `blocked`, and KIS demo account `blocked`.
