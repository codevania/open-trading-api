...
- [[scripts/quant_readiness_check.py|scripts/quant_readiness_check.py]] summarizes local Quant gates without calling KIS/KRX or generating order intents and links the local status coverage audit, lifecycle gap report, evidence collection plan/queue, and source-manifest fill packet into the Point-in-Time status gates, including source manifest validation and missing release/resume group counts when present.
