...
- KRX OpenAPI merged market-data coverage over `2025-01-02` through `2025-03-07` now has `115447` rows across `42` trading dates: `40375` KOSPI rows and `75072` KOSDAQ rows.
...
- Merged KIND status events replayed against the 42-date KRX OpenAPI market-data merge and still marked `705/115447` rows as `exclude_by_status_event`.
...
- [[_report/quant/research/2026-07-08-point-in-time-status-coverage-audit-20250102-20250307-merged-snapshots|_report/quant/research/2026-07-08-point-in-time-status-coverage-audit-20250102-20250307-merged-snapshots.md]] records coverage status `hold`: mode `current_snapshot_smoke`, replay matched `115447/115447` rows, rows with any status-event code `10670`, rows with applied status event `705`, rows excluded by status event `705`, raw status capture dates `2`, and release/resume-like event rows `0`. Lifecycle diagnostics show `managed_issue` `105/0`, `market_alert` `75/0`, and `trading_halt` `253/0` active-like/release-resume rows.
...
- The latest 42-date `Point-in-Time Universe` smoke produced `107766` include rows and `7681` exclude rows.
...
- The latest 42-date, 20-day `Point-in-Time` Liquidity Filter smoke produced `23961` include rows and `91486` exclude rows, with `58815` rows evaluated on the full 20-day lookback.
...
- The latest 42-date, 20-day Liquidity + 5-day Momentum Signal Candidate smoke produced `920` rows across `23` candidate dates: `460` BUY candidates and `460` SELL candidates.
- The latest Signal forward-return smoke produced `1840` horizon rows across `1,5` trading-day horizons: `1600` complete and `240` missing forward prices.
- The latest Signal portfolio target smoke produced `460` long-only paper target rows across `23` rebalance dates at `5%` target weight each.
...
- The latest Backtest PnL smoke is `pass_smoke` with `460` rows, `440` complete rows, and `20` missing forward prices on the latest Signal Candidate date.
...
- [[_report/quant/research/2026-07-08-signal-forward-return-smoke-20d-20250102-20250307|_report/quant/research/2026-07-08-signal-forward-return-smoke-20d-20250102-20250307.md]] records `1840` horizon rows across `1,5` trading-day horizons: `1600` complete and `240` missing forward prices because the smoke window still ends too soon.
