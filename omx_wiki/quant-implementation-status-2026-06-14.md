...
- [[scripts/quant_readiness_check.py|scripts/quant_readiness_check.py]] summarizes local Quant gates without calling KIS/KRX or generating order intents and links the local status coverage audit, lifecycle gap report, evidence collection plan/queue, and source-manifest fill packet into the Point-in-Time status gates, including source manifest validation and missing release/resume group counts when present. The separate fill-progress report is tracking evidence-entry completeness only and does not promote readiness.
...
- [[_report/quant/research/2026-07-10-quant-readiness-check-merged-3snapshots-20d-20250102-20250418-price-through-20250502-with-source-manifest-fill-packet|_report/quant/research/2026-07-10-quant-readiness-check-merged-3snapshots-20d-20250102-20250418-price-through-20250502-with-source-manifest-fill-packet.md]] records the latest readiness gate state with the queue included as `point_in_time_status_evidence_collection_queue=plan_only` and the P1 fill packet included as `point_in_time_status_source_manifest_fill_packet=plan_only`; Backtest readiness remains `hold` and live trading remains `blocked`. The latest fill-progress artifact confirms `0/10` P1 rows are materializable, so this is still a manual official-evidence collection lane.
