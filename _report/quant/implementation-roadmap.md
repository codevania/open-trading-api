...
- Point-in-Time evidence collection plan/queue/fill packet/progress/materializer: latest 3-snapshot local plan organizes `385` pending evidence rows (`10` manifest raw evidence, `314` release/resume evidence, `61` market-label evidence), the local queue groups them into `33` plan-only batches (`P1=10`, `P2=18`, `P3=5`), the P1 fill packet maps the `10` source-manifest batches to fill fields plus non-evidence `source_url_hint` and `raw_path_suggestion` capture aids, the fill-progress report shows `0/10` materializable rows, and the materializer refuses blank/pending rows until official raw capture exists. These are collection targets only and do not count as source coverage.
