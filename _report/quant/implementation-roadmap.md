...
- Current phase: `backtest_input_contract`
- Overall implementation progress: `75-78%`
...
| 7. Backtest engine connection | preflight | 22-25% | [[scripts/quant_backtest_input_contract_validate.py|scripts/quant_backtest_input_contract_validate.py]] validates the 20-day smoke artifacts as internally joinable, while Backtest remains `hold` | Wire engine only after Point-in-Time status coverage is acceptable for the selected scope |
...
- Result: market-data window `pass`, Liquidity Filter `pass_smoke`, Signal Candidate `pass_smoke`, forward-return smoke `pass_smoke`, portfolio target smoke `pass_smoke`, Backtest input contract `pass_smoke`, Point-in-Time status coverage `hold`, Backtest engine `hold`, live trading controls `blocked`, KIS demo account `blocked`.
...
- Quant readiness check: latest 20-day smoke gate report marks market-data window `pass`, Liquidity Filter, Signal Candidate, forward-return, portfolio targets, and Backtest input contract as `pass_smoke`, Point-in-Time status coverage `hold`, Backtest engine `hold`, live trading controls `blocked`, and KIS demo account `blocked`.
