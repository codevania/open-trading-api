...
- Current phase: `portfolio_target_smoke`
- Overall implementation progress: `74-77%`
...
| 7. Backtest engine connection | preflight | 18-22% | [[scripts/quant_readiness_check.py|scripts/quant_readiness_check.py]] reports 20-day smoke gates, including forward-return and portfolio target coverage, while Backtest remains `hold` | Wire engine only after Point-in-Time status coverage is acceptable for the selected scope |
...
| 10. Paper Signal tracking | partial | 55-60% | 20-day Point-in-Time Liquidity rows now feed paper-only Momentum `Signal Candidate`, forward-return, and long-only portfolio target smoke outputs | Keep candidates signal-only until production Point-in-Time coverage, Backtest, OOS, and Bias Control pass |
...
- `Point-in-Time Universe` full historical path is not built; only a 23-date KIND current-snapshot replay smoke exists.
...
Latest merged market-data smoke over `2025-01-02` to `2025-02-07`:
...
- The validated KIND events replayed against the 23-date KRX OpenAPI market-data merge and marked `382/63165` rows as `exclude_by_status_event`.
- The replayed market-data rows were converted into a 23-date `Point-in-Time Universe` smoke with `58961` include rows and `4204` exclude rows.
...
Status: `validated_kind_snapshot_replayed_on_23_date_market_data`
...
- Market data input rows: `63165`
- Status event rows: `344`
- Codes with events: `265`
- Include rows by event state: `62783`
- Exclude rows by event state: `382`
...
- Latest 23-date smoke output: [[_report/quant/research/2026-07-04-kind-status-point-in-time-universe-smoke-20250102-20250207|_report/quant/research/2026-07-04-kind-status-point-in-time-universe-smoke-20250102-20250207.md]]
- Input rows: `63165`
- Include rows: `58961`
- Exclude rows: `4204`
- Exclusion reason counts: `stock_certificate_not_common=2668`, `security_group_not_plain_equity=1154`, `status_event:managed_issue_active=382`
...
- Latest 23-date, 20-day smoke output: [[_report/quant/research/2026-07-04-kind-status-point-in-time-liquidity-smoke-20d-20250102-20250207|_report/quant/research/2026-07-04-kind-status-point-in-time-liquidity-smoke-20d-20250102-20250207.md]]
...
- Result: `4034` include rows and `59131` exclude rows after `avg_trading_value_20d_krw >= 1,000,000,000`; `10236` rows had full 20-day lookback evaluation.
...
- Result: `240` horizon evaluation rows from `120` Signal Candidate rows across `1,5` trading-day horizons; `80` rows complete and `160` rows are `missing_forward_price`.
- Limitation: the current 23-date market-data window ends too soon after latest Signal Candidate dates, so this is a coverage/diagnostic smoke, not a Backtest.
...
- Result: `60` long-only paper target rows across `3` rebalance dates, using `20` BUY candidates per date at `5%` target weight each; `SELL candidate` rows remain exclusions, not short targets.
...
- Result: market-data window `pass`, Liquidity Filter `pass_smoke`, Signal Candidate `pass_smoke`, forward-return smoke `pass_smoke`, portfolio target smoke `pass_smoke`, Point-in-Time status coverage `hold`, Backtest engine `hold`, live trading controls `blocked`, KIS demo account `blocked`.
...
- [[scripts/quant_point_in_time_liquidity_filter.py|scripts/quant_point_in_time_liquidity_filter.py]] also applies a 20-day date-scoped Liquidity Filter to the 23-date Point-in-Time smoke rows.
...
- More daily OHLCV / trading value windows for stability checks beyond the first 23-date smoke range.
...
- KRX OpenAPI date-scoped market-data join/merge: usable as a 23-date smoke input for status replay development.
...
- KIND public status fallback: `344` valid current-snapshot status-event rows replayed against the 23-date KRX OpenAPI market-data merge, marking `382/63165` rows as `exclude_by_status_event`; `310/344` event market labels resolved from the original 17-date join.
...
- Signal forward-return smoke: `120` Signal Candidate rows were evaluated over `1,5` trading-day horizons, producing `240` diagnostic rows with `80` complete and `160` missing forward prices due to the short smoke window.
- Signal portfolio target smoke: `60` long-only paper target rows were generated across `3` rebalance dates at `5%` per selected BUY candidate, with no order intents.
- Quant readiness check: latest 20-day smoke gate report marks market-data window `pass`, Liquidity Filter, Signal Candidate, forward-return, and portfolio targets as `pass_smoke`, Point-in-Time status coverage `hold`, Backtest engine `hold`, live trading controls `blocked`, and KIS demo account `blocked`.
