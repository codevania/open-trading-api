...
## 8. 2026-06-08 Retry Result

사용자 요청으로 인앱 Browser 런타임을 재시도했다.

결과:

- 인앱 Browser가 사용하는 JS 런타임이 `windows sandbox failed: spawn setup refresh`로 시작 단계에서 종료됐다.
- 같은 런타임을 사용하는 Windows Computer Use 경로도 동일 병목으로 판단했다.
- 외부 네트워크 권한을 부여한 PowerShell `Invoke-WebRequest`에서는 KRX 공식 HTML page가 HTTP `200`으로 응답했다.
- HTML에서 `MDCSTAT21401`, `MDCSTAT21501`의 `bld`와 form field를 재확인했다.
- `GenerateOTP` CSV endpoint는 브라우저형 header, `Referer`, `Origin`, main page session 초기화를 포함해도 `LOGOUT`을 반환했다.
- `getJsonData.cmd` JSON endpoint도 같은 자동 session에서 `LOGOUT`으로 실패했다.

판정:

- KRX page 접근과 다운로드 UI 존재는 확인했다.
- 자동 다운로드 이벤트 또는 CSV raw file 확보는 실패했다.
- 현재 상태는 계속 `manual_snapshot_procedure_ready`이며 `download_verified_by_codex: false`다.
