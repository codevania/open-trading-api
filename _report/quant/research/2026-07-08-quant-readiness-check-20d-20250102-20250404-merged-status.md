...
| `benchmark_returns_smoke` | `pass_smoke` | local benchmark return smoke report is diagnostic-only and reports no order intents | join benchmark returns into the Backtest engine only after Point-in-Time and cost gates are ready |
